...
SHORT x,y,n;
{
ClipBlit(&rastport,(long)620,(long)n*10L,&rastport,x,y,(long)10,(long)10,(long)0xC0);
}
Prnt(x,y,n)
USHORT x,y,n;
{
...
ClipBlit(&rastport,(long)0,(long)140,&rastport,(long)x*20+20L,(long)y*19,(long)20,(long)19,(long)0xC0); }

i++;
if(i<2) { if(!i) Audiodzin2(20); else {Audiodzin2(10); } }

if(i==2){Audiopip(20);}
if(i==3){Audiopip(10); i=0;}
...
for(xx=0;xx<6;xx++)
{
 for(x=0;x<30;x++)
 {
  for(y=0;y<13;y++)
  {
   if(k(x,y)>10)
    {
     if(galima((short)53)) { set(x,y,e(x,y)); if(e(x,y)==9)
       { set(x,y,7); e(x,y)=9; }
       }
    }
  }
...
for(x=0;x<30;x++)
{
 for(y=0;y<13;y++)
 {
  if(k(x,y)>10) { set(x,y,e(x,y)); if(e(x,y)==9)
    { set(x,y,7); e(x,y)=9; }
   }
 }
}
...
Move(&rastport,625L,111L); Draw(&rastport,625L,118L);
Move(&rastport,a,121L); Draw(&rastport,625L,128L);
Draw(&rastport,628L,121L);
...
Move(&rastport,a,161L); Draw(&rastport,b,161L); Move(&rastport,625L,161L);
Draw(&rastport,625L,168L);
